...
string jaMayuTotal(string frase3)
{
    for (int i = 0; i < frase3.length(); i++)
    {
        frase3[i] = toupper(frase3[i]);
...
string jaMinuTotal(string frase3)
{
    for (int i = 0; i < frase3.length(); i++)
    {
...
int Anguloseriepalabra07(string frase)
{
    ...
    string frase2 = jaMayuTotal(frase);
...
string jaCredenciales()
{
    ...
    while (contador < 2)
    {
        ...
        if ((nombrenuevo == "javier" && contranueva == "15488354") || (nombrenuevo == "profe" && contranueva == "1234"))
        {
            cout << "Inicio de sesion exitoso. \nbienvenido " << nombrenuevo << endl;
            // Aquí puedes continuar con el resto de tu programa
            return nombrenuevo;
        }

        cout << "Nombre de usuario o clave incorrectos. Intente nuevamente." << endl;
        contador++;
    }

    cout << "Ha excedido el número máximo de intentos" << endl;
    abort();
    // Aquí puedes realizar cualquier acción de aborto que desees
}

bool jaCheckStatus(const bool cabra, const bool lobo, const bool col)
{
    if (cabra == col && cabra != lobo)
        return false;

    return true;
}

bool jaFinalVeri(const bool cabra, const bool lobo, const bool col)
{
    return !cabra && !lobo && !col;
}

void jaAnimacion(const string &objeto, const string &direccion)
{

    cout << "Moviendo " << objeto << " hacia " << direccion << "..." << endl;
    this_thread::sleep_for(std::chrono::seconds(1));

    cout << objeto << " movido hacia " << direccion << endl;
    this_thread::sleep_for(std::chrono::seconds(1));
}
...
void jaCabraColLobo()
{
    ...
    while (true)
    {
        ...
        if (jaFinalVeri(cabra, lobo, col))
        {
            cout << " Has llevado cexito a todos los objetos al otro lado del rio." << endl;
            break;
        }

        jaMenu();

        switch (jagetnumeromas())
        {
        case 1:
            if (cabra)
            {
                jaAnimacion("la cabra", "el otro lado");
                cabra = false;
            }
            else
            {
                cout << "La cabra ya se encuentra en el otro lado del rio." << endl;
            }
            break;
        case 2:
            if (lobo)
            {
                jaAnimacion("el lobo", "el otro lado");
                lobo = false;
            }
            else
            {
        ...
        case 3:
            if (col)
            {
                jaAnimacion("el col", "el otro lado");
                col = false;
            }
            else
            {
                cout << "El col ya se encuentra en el otro lado del rio." << endl;
            }
            break;
        case 4:
            abort();
        ...
        if (!jaCheckStatus(cabra, lobo, col))
        {
            cout << "Has perdido. La cabra y el col se quedaron solos en el mismo lado del rio" << endl;
            break;
        }
    }
}
